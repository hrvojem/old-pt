...
sed -i '18,21 s/^/-- /' /vagrant/world.sql
cat /vagrant/world.sql | mysql -D world
if [ ! -z "$1" ]; then
  if [ "$1" = "ps" ]; then
...
if [ "${WARNINGS_BEFORE}" != "${WARNINGS_AFTER}" ]; then
  echo "There's a difference in number of warnings before installing plugins and after! $WARNINGS_BEFORE $WARNINGS_AFTER"
  exit 1
fi
...
if [ "${ERRORS_BEFORE}" != "${ERRORS_AFTER}" ]; then
  echo "There's a difference in number of errors before installing plugins and after! $ERRORS_BEFORE $ERRORS_AFTER"
  exit 1
fi
